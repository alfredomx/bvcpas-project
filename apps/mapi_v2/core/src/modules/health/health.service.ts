import { Inject, Injectable } from '@nestjs/common'
import type { Sql } from 'postgres'
import type Redis from 'ioredis'
import { DB_CLIENT } from '@/core/db/db.module'
import { REDIS_CLIENT } from '@/core/redis/redis.module'
import { APP_VERSION } from '@/common/version'

export interface ComponentStatus {
  status: 'up' | 'down'
  latency_ms: number
}

/** Shape del liveness check. Crece (plugins cargados) cuando el core los integre. */
export interface HealthStatus {
  status: 'up' | 'degraded'
  version: string
  env: string
  uptime_s: number
  timestamp: string
  components: {
    db: ComponentStatus
    redis: ComponentStatus
  }
}

@Injectable()
export class HealthService {
  constructor(
    @Inject(DB_CLIENT) private readonly db: Sql,
    @Inject(REDIS_CLIENT) private readonly redis: Redis,
  ) {}

  /**
   * Liveness del core: confirma que arrancó y que DB + Redis responden. Los
   * plugins cargados se agregan a `components` cuando entre el plugin-loader.
   */
  async check(): Promise<HealthStatus> {
    const [db, redis] = await Promise.all([this.pingDb(), this.pingRedis()])
    const allUp = db.status === 'up' && redis.status === 'up'
    return {
      status: allUp ? 'up' : 'degraded',
      version: APP_VERSION,
      env: process.env.NODE_ENV ?? 'local',
      uptime_s: Math.floor(process.uptime()),
      timestamp: new Date().toISOString(),
      components: { db, redis },
    }
  }

  private async pingDb(): Promise<ComponentStatus> {
    const start = Date.now()
    try {
      await this.db`select 1`
      return { status: 'up', latency_ms: Date.now() - start }
    } catch {
      return { status: 'down', latency_ms: Date.now() - start }
    }
  }

  private async pingRedis(): Promise<ComponentStatus> {
    const start = Date.now()
    try {
      await this.redis.ping()
      return { status: 'up', latency_ms: Date.now() - start }
    } catch {
      return { status: 'down', latency_ms: Date.now() - start }
    }
  }
}
