...
/**
 * Categoría de una unit montable en el core:
...
/**
 * Manifiesto uniforme de una unit (plugin o pipe). Es la ÚNICA forma en que
 * algo se monta en el core (ver README raíz, regla de oro). El core no conoce
 * las entrañas de la unit: solo su `name`, su `type`, el `module` a montar y
 * el Zod de su config para validarla al boot.
 *
 * Cada plugin/pipe exporta una constante `ModuleDef` y la agrega a `REGISTRY`.
 */
export interface ModuleDef {
  /** Id único de la unit (logs, errores, health). */
   ...
   * (fail-fast agregado). Opcional: una unit sin config se omite.
   */
