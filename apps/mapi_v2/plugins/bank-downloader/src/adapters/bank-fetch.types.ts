 ...
 *
 * El adapter (lógica del banco) no conoce WebSocket: pide un `fetch` a un
 * `BankFetchExecutor`, que lo despacha al plugin (kiro) vía el bridge. El plugin
 * ejecuta el `fetch` en la sesión viva del banco y devuelve el `FetchResult`.
 */
...
/**
 * Respuesta cruda del fetch ejecutado por el plugin. El body es texto (JSON/CSV)
 * o base64 (binario: PDF) según el `content-type`.
 */
 ...
 * (sobre el `BRIDGE_COMMAND_PORT`). En tests: un mock con respuestas canned.
 */
