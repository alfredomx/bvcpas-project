import type { DomStep } from '@/contracts/bridge.port'
 ...
 * **primitivas**: una operación por método, **cero política**. La orquestación
 * (loops, rango, "latest", nombrado, cadencia) vive en `BankDownloadService`.
 *
 * Fechas entran/salen en `MM-DD-YYYY`; cada adapter convierte al formato interno
 * del banco. El transporte es un `BankFetchExecutor`: el adapter pide fetches, el
 * plugin (kiro) los ejecuta en la sesión viva.
 */
 ...
 * sigue tonto. La navegación a `url` la hace mapi con `open_tab`, NO es un paso DOM.
 */
 ...
 * (NO se navega). `steps` son los pasos DOM (click en "Sign out"). Tras el logout,
 * mapi cierra la pestaña con `close_tab`.
 */
...
export abstract class BankAdapter {
  ...
  /** Lista los statements disponibles (metadata, sin PDF). `yearsBack` = años hacia atrás. */
  abstract listStatements(
    accountMask: string,
    opts?: { yearsBack?: number },
  ): Promise<StatementRef[]>

  /** Descarga el PDF de 1 statement. */
  ...
  /**
   * Construye la receta de login (URL + pasos DOM) con las credenciales
   * descifradas. Opcional: un banco sin login automatizado no la implementa →
   * el caller lanza `BankLoginNotSupportedError`.
   */
  buildLoginRecipe?(creds: BankLoginCredentials): BankLoginRecipe

  /**
   * Construye la receta de logout (host del portal + pasos DOM). Opcional: sin
   * logout automatizado → `endSession` lo trata como no-op.
   */
