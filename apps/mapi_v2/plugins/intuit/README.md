...
| `POST`   | `/v1/intuit/clients/:clientId/reconnect`    | re-OAuth del cliente (misma compañía, anti-mixup) | token admin |
| `DELETE` | `/v1/intuit/clients/:clientId/connection`   | desconecta (borra tokens) del cliente             | token admin |
...
`INTUIT_TOKENS_NOT_FOUND` (404) · `INTUIT_REFRESH_EXPIRED` (401) · `INTUIT_STATE_INVALID` (400) · `INTUIT_REALM_CONFLICT` (409) · `INTUIT_REALM_MISMATCH` (409) · `INTUIT_AUTH_ERROR` (502) · `INTUIT_BAD_REQUEST` (400) · `INTUIT_TOO_MANY_RECORDS` (400).
