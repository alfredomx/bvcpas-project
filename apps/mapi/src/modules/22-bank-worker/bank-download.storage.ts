...
/**
 * Guardado a disco de cheques — formato del operador (D-mapi-BW-018).
 *
 * Replica la lógica del plugin original (bankify `image.service.ts`):
 *  - 1 PDF por cheque con la imagen **frontal** (página del tamaño de la imagen).
 *  - Nombre: `MM-DD-YYYY - <checkNumber>.pdf` (fecha de posteo + número de cheque;
 *    si el cheque no trae número, se usa un contador por cuenta — como el original).
 *
 * Estructura: `apps/mapi/.downloads/<cliente>/<mask>/<MM-DD-YYYY - check>.pdf`.
 * El destino real (subir a Dropbox por carpeta del cliente) sigue diferido al BACKLOG.
 */
 ...
 * Con `includeAmount` agrega ` (<amount>)` al final — para depósitos (D-mapi-BW-019).
 * Si el cheque no trae `checkNumber`, usa `fallbackNumber` (contador por cuenta).
 */
 ...
 * (un PDF por cheque, imagen frontal). Salta cuentas sin cheques. Devuelve la
 * carpeta del cliente y cuántos archivos escribió.
 */
 ...
 * `MM-DD-YYYY - <checkNumber> (<amount>).pdf` (CON monto, como bankify). El slip
 * suele no traer checkNumber → usa el contador por cuenta.
 */
...
/**
 * Guarda statements (ya son PDF) como `YYYY-MM.pdf` (como bankify). Solo decodifica
 * el base64 y escribe — sin conversión imagen→PDF.
 */
 ...
 * `<mask> (<from> to <to>).<csv|qbo>` (como bankify). Devuelve la ruta del archivo.
 */
